...
function computeMenuPosition(button, menu, onUpdate) {
  const cleanup = (0, import_dom.autoUpdate)(button, menu, () => __async(null, null, function* () {
...
function createMediaDeviceObserver(kind, onError, requestPermissions = true) {
  ...
  const deviceChanges$ = new import_rxjs2.Observable((subscriber) => {
    var _a;
    const onDeviceChange = () => __async(null, null, function* () {
      try {
...
function setupMediaToggle(source, room, options, publishOptions, onError) {
  ...
  const toggle = (forceState, captureOptions) => __async(null, null, function* () {
    try {
...
function setupManualToggle() {
  ...
  const pendingSubject = new import_rxjs4.Subject();
  const toggle = (forceState) => __async(null, null, function* () {
    pendingSubject.next(true);
...
function setupDeviceSelector(kind, room, localTrack) {
  ...
  const setActiveMediaDevice = (_0, ..._1) => __async(null, [_0, ..._1], function* (id, options = {}) {
    var _a, _b, _c;
    if (localTrack) {
...
function setupDataMessageHandler(room, topic, onMessage) {
  ...
  const send = (_0, ..._1) => __async(null, [_0, ..._1], function* (payload, options = {}) {
...
function setupChatMessageHandler(room) {
  ...
  const send = (text, options) => __async(null, null, function* () {
  ...
  const edit = (text, originalMsg) => __async(null, null, function* () {
...
// src/helper/future.ts
var Future = class {
  constructor(futureBase, onFinally) {
    this._isResolved = false;
    this.onFinally = onFinally;
    this.promise = new Promise((resolve, reject) => __async(this, null, function* () {
      ...
      (_a = this.onFinally) == null ? void 0 : _a.call(this);
    });
  }
  get isResolved() {
...
var streamIdToAttachments = /* @__PURE__ */ new Map();
...
function setupChat(room, options) {
  ...
  if (needsSetup) {
    room.registerTextStreamHandler(topic, (reader, participantInfo) => __async(null, null, function* () {
      const { id, timestamp, attributes, attachedStreamIds } = reader.info;
      const attachments = new Map(
        (attachedStreamIds != null ? attachedStreamIds : []).map((id2) => [
          id2,
          new Future()
        ])
      );
      ...
      const streamObservable = (0, import_rxjs7.from)(reader).pipe(
        ...
        (0, import_rxjs7.mergeMap)((chunk2) => {
          if (attachments.size === 0) {
            return (0, import_rxjs7.of)({ chunk: chunk2, attachedFiles: [] });
          } else {
            return (0, import_rxjs7.from)(attachments.values()).pipe(
              (0, import_rxjs7.mergeMap)((attachment) => (0, import_rxjs7.from)(attachment.promise)),
              (0, import_rxjs7.scan)(
                (acc, attachment) => [...acc, new File(attachment.buffer, attachment.fileName)],
                []
              ),
              (0, import_rxjs7.map)((attachedFiles) => ({ chunk: chunk2, attachedFiles }))
            );
          }
        }),
        (0, import_rxjs7.map)(({ chunk: chunk2, attachedFiles }) => {
          return {
            id,
            timestamp,
            message: chunk2,
        ...
        (0, import_rxjs7.finalize)(() => streamIdToAttachments.delete(id))
      );
      streamObservable.subscribe({
        next: (value) => messageSubject.next(value)
      });
    }));
    room.registerByteStreamHandler(topic, (reader) => __async(null, null, function* () {
      var _a2, _b2;
      const { id: attachmentStreamId } = reader.info;
      const foundStreamAttachmentPair = Array.from(streamIdToAttachments).find(
        ([, attachments]) => attachments.has(attachmentStreamId)
      );
      ...
      const bufferList = [];
      try {
        for (var iter = __forAwait(reader), more, temp, error; more = !(temp = yield iter.next()).done; more = false) {
          const buffer = temp.value;
      ...
      const attachment = (_a2 = streamIdToAttachments.get(streamId)) == null ? void 0 : _a2.get(attachmentStreamId);
      if (!attachment) {
        return;
      }
      (_b2 = attachment.resolve) == null ? void 0 : _b2.call(attachment, {
  ...
  const send = (message, options2) => __async(null, null, function* () {
    var _a2;
    if (!options2) {
      options2 = {};
    }
...
function setupStartAudio() {
  const handleStartAudioPlayback = (room) => __async(null, null, function* () {
...
function setupStartVideo() {
  const handleStartVideoPlayback = (room) => __async(null, null, function* () {
